...
class ServerFailureRequestRetry(
    dispatcher: CoroutineDispatcher = Dispatchers.Default,
    val serverFailureRetry: CustomNetworkRequest.RequestRetryAttempt
) : RetryStrategy {
    ...
    override fun processRetry(block: () -> Unit) {
        if (count < serverFailureRetry.attempt) {
            ++count
            job = networkRetryScope.launch {
                delay(serverFailureRetry.requestIntervalInMillis)
                block()
            }
        }
    }
